...
unsafe fn inspect_adapter(
    ...
    let supported_extensions = supported_extension_properties
        .iter()
        .map(|ext_prop| {
            unsafe { ffi::CStr::from_ptr(ext_prop.extension_name.as_ptr()) }
        })
        .collect::<Vec<_>>();
    ...
    let device_information = crate::DeviceInformation {
        ...
        device_name: unsafe { ffi::CStr::from_ptr(properties.device_name.as_ptr()) }
            .to_string_lossy()
            .to_string(),
        driver_name: unsafe { ffi::CStr::from_ptr(driver_properties.driver_name.as_ptr()) }
            .to_string_lossy()
            .to_string(),
        driver_info: unsafe { ffi::CStr::from_ptr(driver_properties.driver_info.as_ptr()) }
            .to_string_lossy()
            .to_string(),
    };
...
impl super::Context {
    pub fn init(desc: crate::ContextDesc) -> Result<Self, NotSupportedError> {
        ...
        let physical_devices = unsafe { instance
            .core
        ...
        let memory_manager = {
            let mem_properties = unsafe { instance
                .core
        ...
        let timeline_semaphore = unsafe { device
            .core
